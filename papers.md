...
{% for paper in sorted %}
<a href="{{ paper.url | relative_url }}" class="block py-4 border-b border-black/10 first:pt-0 hover:text-bolt transition-colors">
  <p class="text-xs text-black/40 mb-0.5">{{ paper.date | date: '%Y-%m-%d' }} / {{ paper.arxiv_id }}</p>
